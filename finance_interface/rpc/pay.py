...
import requests
import xmltodict
from ..ext_libs.weixin.helper import smart_str, smart_bytes, smart_text, md5_constructor as md5
...
class WeixinPay(object):
    ...
    def prepare_request(self, method, path, params):
        ...
        if self.mch_cert:
            if self.mch_key:
                kwargs['cert'] = (
                    self.mch_cert, self.mch_key)
        return (
            method, url, kwargs)

    def unifiedorder(self, body='', out_trade_no='', total_fee='', openid='', detail='', attach='', time_start='',
                     time_expire='', goods_tag='', product_id='', limit_pay='', device_info='', fee_type='CNY',
                     spbill_create_ip=SPBILL_CREATE_IP, trade_type='JSAPI', notify_url=''):
        """
        统一下单接口
    ...
    def refund(self, out_refund_no, total_fee, refund_fee, op_user_id, out_trade_no='', transaction_id='',
    ...
    def refundquery(self, out_trade_no='', transaction_id='', out_refund_no='', refund_id='', device_info=''):
        """
        退款查询接口
...
class WeixinAppPay(WeixinPay):
    ...
    def get_base_params(self, is_app=False):
        params = {'appid': self.appid}
        if is_app:
            params.update({'partnerid': self.mch_id,
                           'noncestr': generate_nonce_str()})
        else:
            params.update({'mch_id': self.mch_id,
                           'nonce_str': generate_nonce_str()})
        return params
...
class WXAppPay(WeixinPay):
    ...
    def unifiedorder(self, body='', out_trade_no='', total_fee='', openid='', detail='', attach='', time_start='',
        ...
        result.update({'sign': sign_params.get('sign'),
...
class WeixinEnterprisePay(WeixinPay):
    ...
    def transfers(self, partner_trade_no, openid, amount, desc, spbill_create_ip=SPBILL_CREATE_IP,
                  check_name='NO_CHECK', re_user_name='', device_info=''):
        """
        企业付款接口
        https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_1
...
def build_pay_sign(app_id, nonce_str, prepay_id, time_stamp, key, signType='MD5'):
    ...
    :return:
    """
    sign = 'appId={app_id}&nonceStr={nonce_str}&package=prepay_id={prepay_id}&signType={signType}&timeStamp={time_stamp}&key={key}'.format(
        app_id=app_id,
        nonce_str=nonce_str,
        prepay_id=prepay_id,
