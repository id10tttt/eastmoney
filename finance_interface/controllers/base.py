...
class BaseController(object):
    ...
    def _check_domain(self, sub_domain):
        wxapp_entry = request.env['wx.config'].sudo().get_entry(sub_domain)
        if not wxapp_entry:
    ...
    def _check_user_is_vip(self, wechat_user):
        """
        判断是不是VIP客户
        """
        return request.env['wx.subscribe.order'].wx_user_is_vip(wechat_user)
    ...
    def _check_user(self, sub_domain, token):
        ...
        if login_uid:
            if str(login_uid) == token:  # request.session.sid==token:
                wechat_user = request.env['wx.user'].sudo().search(
        ...
        access_token = request.env['wx.access.token'].sudo().search([
            ('token', '=', token),
            # ('create_uid', '=', user.id)
        ])
    ...
    def check_userid(self, token):
        if token:
            ...
            access_token = request.env['wx.access_token'].sudo().search([
                ('token', '=', token),
            ])
            if not access_token:
                return
